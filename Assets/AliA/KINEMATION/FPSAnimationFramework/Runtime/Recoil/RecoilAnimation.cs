...
namespace KINEMATION.FPSAnimationFramework.Runtime.Recoil
{
    ...
    public class RecoilAnimation : MonoBehaviour
    {
        ...
        {
            _noiseTarget.x = Mathf.Lerp(_noiseTarget.x, 0f, KMath.ExpDecayAlpha(_recoilData.noiseDamp.x, Time.deltaTime));
            _noiseTarget.y = Mathf.Lerp(_noiseTarget.y, 0f, KMath.ExpDecayAlpha(_recoilData.noiseDamp.y, Time.deltaTime));
	
            _noiseOut.x = Mathf.Lerp(_noiseOut.x, _noiseTarget.x, 
                KMath.ExpDecayAlpha(_recoilData.noiseAccel.x, Time.deltaTime));

            _noiseOut.y = Mathf.Lerp(_noiseOut.y, _noiseTarget.y, 
                KMath.ExpDecayAlpha(_recoilData.noiseAccel.y, Time.deltaTime));
        ...
        private void ApplyPushback(ref Vector3 finalized)
        {
            _pushTarget = Mathf.Lerp(_pushTarget, 0f, 
                KMath.ExpDecayAlpha(_recoilData.pushDamp, Time.deltaTime));
            
            _pushOut = Mathf.Lerp(_pushOut, _pushTarget, 
                KMath.ExpDecayAlpha(_recoilData.pushAccel, Time.deltaTime));
